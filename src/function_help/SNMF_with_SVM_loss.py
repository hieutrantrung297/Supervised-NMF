...
def update_weight(u, K, weight, learning_rate = 0.001):
    X_ext = extend_features(K)
    
    n = u.shape[0]
    for iter in range(n):
        gradient = compute_gradients(weight, X_ext, u)
...
def hinge_loss(w, X, y, lambda_param=0.01):
    n = X.shape[0]
    distances = 1 - y * np.dot(X, w)
    distances = np.maximum(0, distances)
    hinge_loss = (1 / n) * (np.sum(distances) + 0.5 * lambda_param * np.dot(w, w) - 0.5 * lambda_param * np.dot(w[-1], w[-1]))
    return hinge_loss

def compute_gradients(w, X, y, lambda_param=0.01):
...
def cost_function(Y, K, X, u = np.array([]), weight = np.array([])):
    ...
    if len(u) == 0 or len(weight) == 0:
        ...
        loss2 = hinge_loss(weight, X_ext, u)
...
def SNMF(Y, u, rank, iter, tolerance, patience, epsStab, alpha, epsilon, init_mode):
    ...
    # Weight
    W = extend_features(K)
    weight = np.random.randn(W.shape[1])
    ...
    for i in range(iter):
        ...
        # Update weight
        weight = update_weight(u, K, weight)
